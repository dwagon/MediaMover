...
default_srcdir = "/downloads/complete/"
default_dstdir = "/Media/TV"
    ...
    for series in tvdb_show:
        ...
        for episode in episodes:
            if (
                ...
                and episode["airedEpisodeNumber"] == episodenum
            ):
...
@click.command()
@click.option(
    "-k", "--kidding", default=False, is_flag=True, help="Don't actually do anything"
)
@click.option(
    "--srcdir",
    default=default_srcdir,
    help="Where to get files from",
    envvar="MEDIA_SRCDIR",
)
@click.option(
    "--destdir",
    default=default_dstdir,
    help="Where to put files to",
    envvar="MEDIA_DSTDIR",
)
...
def cli(
    kidding, srcdir, destdir, username, userkey, apikey
